...
export function normalizeQuotes(rows, market) {
  ...
  for (const row of rows) {
    ...
    out.push({
      id, name, market, date, price, change, previousClose: previous, quoteType: 'end-of-day',
      volume: volume !== null && volume >= 0 ? volume / 1000 : null,
      amount: amount !== null && amount >= 0 ? amount : null,
      currency: 'TWD', volumeUnit: 'thousand-shares',
...
export async function finmind(dataset, params = {}) {
  ...
  const data = await fetchJson(url.toString(), { timeout: 20000, headers: token ? { Authorization: `Bearer ${token}` } : {} });
...
export async function getHistory(id, market = 'TW') {
  ...
  if (['TW', 'TWSE', 'TPEX'].includes(market)) {
    if (!/^\d{4,6}[A-Z]?$/.test(id)) throw new HttpError('\u53f0\u80a1\u6b77\u53f2\u65e5\u7dda\u53ea\u63a5\u53d7 4-6 \u78bc\u4ee3\u865f\u3002', 400);
