...
function realtimeSymbols(){
 ...
 return [...ids].filter(id=>/^\d{4,6}$/.test(id)).slice(0,20);
}
...
async function syncRealtime(options={}){
 ...
 try{
  ...
  for(const q of res.quotes){
   ...
   liveQuotes[q.id]={...old,price:finite(q.price)?q.price:old.price,change:finite(q.change)?q.change:old.change,volume:finite(q.volume)?q.volume:old.volume,amount:finite(q.amount)?q.amount:old.amount,date:String(q.date||old.date||''),source:String(q.source||old.source||'TWSE MIS realtime'),realtime:true,realtimeTime:String(q.time||'')};
   applyRealtimeBar(q);updated++;
  }
  if(updated){mode='official';realtimeLastAt=res.fetchedAt||new Date().toISOString();liveMeta={...(liveMeta||{}),label:`${res.label||'TWSE MIS 行情'}${res.quotes[0]?.time?' · '+res.quotes[0].time:''}`,fetchedAt:realtimeLastAt,realtime:true};analyticsCache.clear();render();if(selectedStock&&stock(selectedStock))drawStock();checkAlerts();}
  if(options.force)toast(updated?`即時更新 ${updated} 檔；盤中約每 5 秒自動刷新。`:'沒有可套用的即時標的。');
  return updated>0;
 }catch(e){
...
function scheduleRealtimePolling(){
 ...
 realtimeTimer=setInterval(()=>{if(document.hidden||fx.market!=='TW'||!twMarketOpen())return;syncRealtime().catch(()=>{});},5000);
}
...
notice=function(){const rt=mode==='official'&&liveMeta?.realtime;return `<div class="notice ${mode==='demo'?'demo-notice':''}">${icon('info')}<span>${mode==='demo'?'<strong>操作示範</strong>　價格、K線與訊號全部由合成資料計算，D001 等為序號，不是真實行情。':mode==='unconnected'?'<strong>尚未連接行情</strong>　不顯示假報價或替代K線。請連接後端或匯入你的歷史資料。':rt?'<strong>台股即時模式</strong>　TWSE MIS 約 5 秒刷新；僅供個人研究。盤中技術指標含當日暫時 K 棒，收盤前仍可能變動。':'<strong>資料模式：'+(mode==='imported'?'使用者匯入':'外部盤後行情')+'</strong>　依每檔來源與資料日期顯示；缺值不補造。'} <button data-action="nav" data-view="settings">資料設定 ↗</button></span></div>`;};
